...
namespace RoninEngine::Runtime
{
    ...
    */
    ...
    bool order_by_layer(Renderer *const lhs, Renderer *const rhs)
    {
        return lhs->transform()->_layer_ < rhs->transform()->_layer_;
    }

    using OrderTransformLayer = std::integral_constant<decltype(&order_by_layer), &order_by_layer>;
    ...
    void native_render_2D(Camera2D *camera)
    {
        struct
        {
            ...
            int edges;
            std::multiset<Renderer *, OrderTransformLayer> layers;
        } stack;
                        ...
                        for(auto layerObject : layer->second)
                        {
                            for(Transform *iter : layerObject.second)
                            {
                                    ...
                                    ++component)
                                {
                                    ...
                                    stack.layers.insert(render_iobject);
                                }
                            }
                        }
                    }
...
#undef MX
        ...
        for(Renderer *render_iobject : stack.layers)
        {
