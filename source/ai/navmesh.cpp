...
#define MEMORY_DATA (((std::uint8_t*)neurons) + ((range.x * heightSpace + range.y) * NavMeshDataSizeMultiplier) + segmentOffset)
    ...
    public:
        ...
        static void AvailPoints(NavMesh& map, NavMethodRule method, Runtime::Vec2Int arrange, Runtime::Vec2Int target, std::list<Runtime::Vec2Int>* pathTo, std::size_t maxCount = -1, int filterFlag = -1);
    };
    ...
    bool RoninEngine::AIPathFinder::NavMesh::neuronContains(const Runtime::Vec2Int& range) { return range.x > ~0 && range.y > ~0 && range.x <= widthSpace && range.y <= heightSpace; }
    ...
    const Vec2Int NavMesh::neuronGetPoint(const Neuron* neuron)
    {
        ...
        auto divide = std::div((reinterpret_cast<std::size_t>(neuron) - reinterpret_cast<std::size_t>(neurons) - segmentOffset) / NavMeshDataSizeMultiplier, heightSpace);
    ...
    void NavMesh::load(const NavMeshData& navmeshData)
    {
        ...
        if (navmeshData.neurons == nullptr)
    ...
    void NavMesh::find(NavResult& navResult, NavMethodRule method, Neuron* firstNeuron, Neuron* lastNeuron, NavAlgorithm algorithm) { find(navResult, method, neuronGetPoint(firstNeuron), neuronGetPoint(lastNeuron), algorithm); }
    ...
    void AlgorithmUtils::AvailPoints(NavMesh& map, NavMethodRule method, Vec2Int arrange, Vec2Int target, std::list<Vec2Int>* pathTo, std::size_t maxCount, int filterFlag)
    {
        Neuron* it = nullptr;
        Vec2Int point;
        int i = 0, c;
