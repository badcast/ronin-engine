...
#define MEMORY_DATA                                                  \
  (((std::uint8_t *)neurons) +                                       \
   ((range.x * heightSpace + range.y) * NavMeshDataSizeMultiplier) + \
   segmentOffset)
...
NavMesh::NavMesh(int lwidth, int lheight) {
  ...
  auto lockedDiv =
      div(Math::max(lheight = (lwidth * lheight), 8), 8);  // add locked bits
...
void NavMesh::stress() {
  NavResult result;
  Vec2Int first, next = {static_cast<int>(widthSpace - 1),
                         static_cast<int>(heightSpace - 1)};
  // TODO: next a strees
...
Neuron *NavMesh::GetNeuron(int x, int y) { return GetNeuron(Vec2Int(x, y)); }
...
const Vec2Int NavMesh::WorldPointToPoint(const Vec2 &worldPoint) {
  Vec2Int p;
  p.x = Math::ceil(widthSpace / 2 +
                   (worldPoint.x + worldOffset.x) / worldScale.x);
  p.y = Math::ceil(heightSpace / 2 -
                   (worldPoint.y + worldOffset.y) / worldScale.y);
  return p;
}
...
void NavMesh::find(NavResult &navResult, NavMethodRule method,
                   Runtime::Vec2 worldPointFirst,
                   Runtime::Vec2 worldPointLast) {
  find(navResult, method, this->WorldPointToPoint(worldPointFirst),
       WorldPointToPoint(worldPointLast), NavAlgorithm::AStar);
}
...
Neuron *NavMesh::GetNeuron(const Runtime::Vec2 &worldPoint,
                           Runtime::Vec2Int &outPoint) {
  outPoint.x = Math::ceil(widthSpace / 2 +
                          (worldPoint.x + worldOffset.x) / worldScale.x);
  outPoint.y = Math::ceil(heightSpace / 2 -
                          (worldPoint.y + worldOffset.y) / worldScale.y);
  return GetNeuron(outPoint);
}

const Runtime::Vec2 NavMesh::PointToWorldPosition(
    const Runtime::Vec2Int &range) {
  return PointToWorldPosition(range.x, range.y);
}
const Runtime::Vec2 NavMesh::PointToWorldPosition(Neuron *neuron) {
  return PointToWorldPosition(neuronGetPoint(neuron));
}
...
bool RoninEngine::AIPathFinder::NavMesh::neuronLocked(
    const Runtime::Vec2Int &range) {
...
bool RoninEngine::AIPathFinder::NavMesh::neuronContains(
    const Runtime::Vec2Int &range) {
  return range.x > ~0 && range.y > ~0 && range.x <= widthSpace &&
         range.y <= heightSpace;
}

std::uint8_t &RoninEngine::AIPathFinder::NavMesh::neuronGetFlag(
    const Runtime::Vec2Int &range) {
...
std::uint32_t &RoninEngine::AIPathFinder::NavMesh::neuronGetCost(
    const Runtime::Vec2Int &range) {
  Neuron *n = GetNeuron(range);
  if (!n) throw std::out_of_range("range");
  return n->cost;
...
std::uint32_t &RoninEngine::AIPathFinder::NavMesh::neuronHeuristic(
    const Runtime::Vec2Int &range) {
  Neuron *n = GetNeuron(range);
  if (!n) throw std::out_of_range("range");
  return n->h;
...
const int RoninEngine::AIPathFinder::NavMesh::neuronGetWeight(
    const Runtime::Vec2Int &range) {
  if (!neuronContains(range)) throw std::out_of_range("range");
  return range.x * range.y + range.y * range.y;
}

const std::uint32_t RoninEngine::AIPathFinder::NavMesh::neuronGetTotal(
    const Runtime::Vec2Int &range) {
  Neuron *n = GetNeuron(range);
  if (!n) throw std::out_of_range("range");
  return n->cost + n->h;
...
const bool RoninEngine::AIPathFinder::NavMesh::neuronEmpty(
    const Runtime::Vec2Int &range) {
  return !neuronGetTotal(range);
}

bool NavMesh::neuronLocked(const Neuron *neuron) {
  return neuronLocked(neuronGetPoint(neuron));
}
std::uint8_t &NavMesh::neuronGetFlag(const Neuron *neuron) {
  return neuronGetFlag(neuronGetPoint(neuron));
}
std::uint32_t &NavMesh::neuronGetCost(const Neuron *neuron) {
  return neuronGetCost(neuronGetPoint(neuron));
}
std::uint32_t &NavMesh::neuronHeuristic(const Neuron *neuron) {
  return neuronHeuristic(neuronGetPoint(neuron));
}
const int NavMesh::neuronGetWeight(const Neuron *neuron) {
  return neuronGetWeight(neuronGetPoint(neuron));
}
const std::uint32_t NavMesh::neuronGetTotal(const Neuron *neuron) {
  return neuronEmpty(neuronGetPoint(neuron));
}
const bool NavMesh::neuronEmpty(const Neuron *neuron) {
  return neuronEmpty(neuronGetPoint(neuron));
}
void NavMesh::neuronLock(const Neuron *neuron, const bool state) {
...
const Vec2Int NavMesh::neuronGetPoint(const Neuron *neuron) {
  ...
  auto divide =
      std::div((reinterpret_cast<std::size_t>(neuron) -
                reinterpret_cast<std::size_t>(neurons) - segmentOffset) /
                   NavMeshDataSizeMultiplier,
               heightSpace);
  return {divide.quot, divide.rem};
}

void RoninEngine::AIPathFinder::NavMesh::neuronLock(
    const Runtime::Vec2Int &range, const bool state) {
  auto divide = std::div(range.x * heightSpace + range.y, 8);
  auto &&pointer =
      (reinterpret_cast<std::uint8_t *>(neurons) + segmentOffset) + divide.quot;
...
void NavMesh::load(const NavMeshData &navmeshData) {
  ...
  if (this->neurons == navmeshData.neurons)
    throw std::runtime_error("neurons == navmesh data, unflow effect");
  ...
  memcpy(this->neurons, navmeshData.neurons,
         widthSpace * heightSpace * sizeof(Neuron));
}
...
void NavMesh::find(NavResult &navResult, NavMethodRule method,
                   Neuron *firstNeuron, Neuron *lastNeuron,
                   NavAlgorithm algorithm) {
  find(navResult, method, neuronGetPoint(firstNeuron),
       neuronGetPoint(lastNeuron), algorithm);
}

void NavMesh::find(NavResult &navResult, NavMethodRule method, Vec2Int first,
                   Vec2Int last, NavAlgorithm algorithm) {
  ...
  if ((firstNeuron = GetNeuron(first)) == nullptr ||
      (lastNeuron = GetNeuron(last)) == nullptr) {
  ...
  while (!navResult.RelativePaths.empty()) {
    ...
    AlgorithmUtils::AvailPoints(*this, method, this->neuronGetPoint(current),
                                last, &finded);
  ...
  while (current != firstNeuron) {
    ...
    lastSelect = current;
    AlgorithmUtils::AvailPoints(*this, method, neuronGetPoint(current), first,
                                &finded, -1, FLAG_OPEN_LIST | FLAG_CLOSED_LIST);
    for (iter = std::begin(finded); iter != std::end(finded); ++iter) {
      select = GetNeuron(*iter);
      if ((select->cost && select->cost < current->cost) ||
          select == firstNeuron) {
        current = select;
...
int AlgorithmUtils::DistancePhf(const Vec2Int &a, const Vec2Int &b) {
  return Math::pow(a.x - b.x, 2) +
         Math::pow(a.y - b.y, 2);  // a->x * a->y + b->x * b->y;
}

int AlgorithmUtils::DistanceManht(const Vec2Int &a, const Vec2Int &b) {
  return Math::abs(b.x - a.x) + Math::abs(b.y - a.y);
}

auto AlgorithmUtils::GetMinCostPath(NavMesh &map, std::list<Vec2Int> *paths)
...
int GetMatrixMethod(NavMethodRule method, std::int8_t **matrixH,
                    std::int8_t **matrixV) {
...
void AlgorithmUtils::AvailPoints(NavMesh &map, NavMethodRule method,
                                 Vec2Int arrange, Vec2Int target,
                                 std::list<Vec2Int> *pathTo,
  ...
  switch (method) {
    case RoninEngine::AIPathFinder::NavMethodRule::NavigationIntelegency: {
      ...
      do {
        ...
        if (it && !map.neuronLocked(point) &&
            (filterFlag == ~0 || it->flags & filterFlag)) {
