...
const int Texture::height() {
    int h;
    if (SDL_QueryTexture(m_native, nullptr, nullptr, nullptr, &h)) Application::back_fail();
    return h;
}

const SDL_PixelFormatEnum Texture::format() {
    uint32_t f;
    if (SDL_QueryTexture(m_native, &f, nullptr, nullptr, nullptr)) Application::back_fail();
...
const SDL_BlendMode Texture::blendMode() {
    ...
    if (SDL_GetTextureBlendMode(m_native, &bmode)) Application::back_fail();
...
const void Texture::scaleMode(const SDL_ScaleMode scaleMode) {
    if (SDL_SetTextureScaleMode(m_native, scaleMode)) Application::back_fail();
}

const SDL_TextureAccess Texture::access() {
    int ac;
    if (SDL_QueryTexture(m_native, nullptr, &ac, nullptr, nullptr)) Application::back_fail();
    return static_cast<SDL_TextureAccess>(ac);
}

const Color Texture::color() {
    Color c;
    if (!(!SDL_GetTextureColorMod(m_native, &c.r, &c.g, &c.b) && !SDL_GetTextureAlphaMod(m_native, &c.a))) Application::back_fail();

    return c;
}
const void Texture::color(const Color value) {
    if (!(!SDL_SetTextureColorMod(m_native, value.r, value.g, value.b) && !SDL_SetTextureAlphaMod(m_native, value.a))) Application::back_fail();
}
