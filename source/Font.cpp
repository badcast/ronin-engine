...
void Initialize_Fonts(bool optimizeDeffects) {
    ...
    if (GC::gc_alloc_lval(pLegacyFont) == nullptr) Application::fail_OutOfMemory();

    if ((pLegacyFont->surfNormal = GC::GetSurface("font-arealike")) == nullptr) throw std::bad_alloc();

    if ((pLegacyFont->surfHilight = GC::GetSurface("font-arealike-hi")) == nullptr) throw std::bad_alloc();
    ...
    //Структурирование шрифта
    {
        ...
        for (i = 0; target != 127;) {
            ...
            p->h = pLegacyFont->fontSize.y;
            ++i;
            if (!(i = i % maxWidth) || target == 127) deltay += pLegacyFont->fontSize.y;
        }
        ...
        for (i = 0; target != 255;) {
            ...
            p->h = pLegacyFont->fontSize.y;
            ++i;
            if (!(i = i % maxWidth)) deltay += pLegacyFont->fontSize.y;
        }
    }

    // convert surface to texture
    GC::gc_alloc_sdl_texture(&pfontTexture, pLegacyFont->surfNormal);
    GC::gc_alloc_sdl_texture(&pfontTextureHilight, pLegacyFont->surfHilight);
...
void Render_String(SDL_Renderer *renderer, Rect rect, const char *text, int len, int pfontWidth, TextAlign textAlign, bool textWrap, bool hilight) {
    ...
    if (!rect.h) rect.h = pLegacyFont->fontSize.y;  // todo: для мультий строк требуется вычислить h высоту
