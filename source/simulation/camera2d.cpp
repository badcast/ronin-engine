...
RoninEngine::Runtime::Camera2D::Camera2D(const Camera2D& proto)
    : Camera(proto.m_name), visibleBorders(proto.visibleBorders), visibleGrids(proto.visibleGrids), visibleObjects(proto.visibleObjects), scale(proto.scale) {}
...
void Camera2D::render(SDL_Renderer* renderer, Rect rect, GameObject* root) {
    ...
    for (const auto& layer : *(std::get<0>(filter)))
        for (auto renderSource : layer.second) {
            ...
            if (wrapper.texture) {
                ...
                if (arranged != Vec2::zero) arranged = Vec2::RotateAround(sourcePoint, arranged, renderSource->transform()->angle() * Math::Deg2Rad);
                ...
                SDL_RenderCopyExF(renderer, wrapper.texture->native(), (SDL_Rect*)&wrapper.src, reinterpret_cast<SDL_FRect*>(&wrapper.dst),
                                  renderSource->transform()->angle(), nullptr, SDL_RendererFlip::SDL_FLIP_NONE);
            }
        }
    // Render Lights
    for (auto lightSource : *std::get<1>(filter)) {
        ...
        if (wrapper.texture) {
            ...
            SDL_RenderCopyExF(renderer, wrapper.texture->native(), reinterpret_cast<SDL_Rect*>(&wrapper.src), reinterpret_cast<SDL_FRect*>(&wrapper.dst),
